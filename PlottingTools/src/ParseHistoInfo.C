...
std::vector<HistoInfo> ParseHistoInfo::ExtractHistoInformation(char* histcardname){
  ...
  while( std::getline( histfile, Line ) )    {  // output of readline is stored in "line"
	...
	if(Rebin.size() == 0) {
      	  std::cout<<" You have not provided Rebin, if you want this then fix the plotting card"<<std::endl;
      	}
      }//if(resultHist=="Histname"){
      ...
      if(resultHist=="isLog"){
	std::string isLog;
	while( std::getline( issHist, isLog) ){
	  islog = atoi(isLog.c_str());
	}
	if(debug)	std::cout<<" islog = "<<islog<<std::endl;
	if(isLog.size() == 0) {
      	  std::cout<<" You have not provided isLog, if you want this then fix the plotting card"<<std::endl;
      	}
      }//if(resultHist=="Histname"){
      ...
      //Extract isfillcolor
      if(resultHist=="isfillcolor"){
	std::string isfillcolor;
	while( std::getline( issHist, isfillcolor) ){
	  IsFillColor = atoi(isfillcolor.c_str());
	}
	if(debug)	std::cout<<" isfillcolor = "<<IsFillColor<<std::endl;
	if(isfillcolor.size() == 0) {
      	  std::cout<<" You have not isfillcolor, if you want this then fix the plotting card"<<std::endl;
      	}
      }//if(resultHist=="isfillcolor"){
	...
	if(colors.size() == 0 && Histname.size() > 1){  // use colors from Rootfile info if there are >1 files
      ...
      //Extract fill style
      if(resultHist=="fillstyle"){
	std::string fillstyle;
	int FillStyle;
	while( std::getline( issHist, fillstyle,' ') ){
	  FillStyle = atoi(fillstyle.c_str());
	  if(debug) std::cout<<" FillStyle = "<<FillStyle<<std::endl;
	  FillStyles.push_back(FillStyle);
	}
	if(FillStyles.size() == 0 && IsFillColor > 0){  
	  std::cout<<" Please provide fill style Informations .. fix the plotting card"<<std::endl;
	  return histInfoVector;
	}
      }//if(result=="color"){                                                                                                                                        
